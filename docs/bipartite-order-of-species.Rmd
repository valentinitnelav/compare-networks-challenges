...
And these are the network metrics that one can compute:
...
Now, if we shuffle the insect species names in `Safariland`, we get differences in some network metrics. We do not alter the structure of the network, the abundance information stays unchanged, we just randomly input data in a different order (could be whatever order we typed in the species in the Excel file).
...
Try also to order alphabetically the column names
